...
The first input parameter of the program CRandTest is the position where we choose the bit from the return value of rand() in each time. The legal value of first parameter is from 1 to 15. The second input parameter is the seed file for srand function, it should be uncompressed from file 10GroupsRandomSeeds.rar in the upper folder.   
  
   Experimental results of CRandTest for position x are in the generated file "C_Rand_posx_result.txt"(if x is 1, then the result file is "C_Rand_pos1_result.txt"), including both P and U results, and the number of rejected sequences according to the criteria of U.
